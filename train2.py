...
def train(net: nn.Module, dataloader: DataLoader, optimizers: dict, device: torch.device, epoch, print_freq=100):
...
########################### Debug ##############################
# word_map = json.load(open('./data_config/SelectGloveWordMap.json'))
...
def save_checkpoint(epoch, net: nn.Module, optimizers: dict, bleu4, is_best, epoches_since_improvement, out_folder='./checkpoints'):
    state = {
        'epoch': epoch,
        'model': net,
        'optimizers': optimizers,
        'bleu4': bleu4,
        'epoches_since_improvement': epoches_since_improvement
    }
...
if __name__ == "__main__":
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=train_batch_size, shuffle=True, num_workers=train_num_workers, collate_fn=train_dataset.collate_fn, pin_memory=pin_memory)
    valid_dataloader = DataLoader(valid_dataset, batch_size=valid_batch_size, shuffle=False, num_workers=valid_num_workers, collate_fn=valid_dataset.collate_fn, pin_memory=pin_memory)
    ...
    # train_dataloader = DataLoader(train_dataset, batch_size=train_batch_size, shuffle=False, num_workers=train_num_workers, collate_fn=train_dataset.collate_fn, pin_memory=pin_memory)
    # valid_dataloader = DataLoader(valid_dataset, batch_size=valid_batch_size, shuffle=False, num_workers=valid_num_workers, collate_fn=valid_dataset.collate_fn, pin_memory=pin_memory)
    ########################### Debug ##############################
    ...
    if checkpoint is None:
        ...
        optimizers = {
            'encoder': optim.Adam(params=filter(lambda p: p.requires_grad, net.encoder.parameters()), lr=encoder_lr) if fine_tune_encoder else None,
            'decoder': optim.Adam(params=filter(lambda p: p.requires_grad, net.decoder_with_attention.parameters()), lr=decoder_lr)
        }
        best_bleu4 = 0
        epoches_since_improvement = 0
        ...
        epoches_since_improvement = checkpoint['epoches_since_improvement']

        ########################### Debug #############################
        # adjust_learning_rate(optimizers['decoder'], 0.1)
        ########################### Debug #############################

        if fine_tune_encoder and optimizers['encoder'] is None:
            net.encoder.fine_tune(True)
            optimizers['encoder'] = optim.Adam(params=filter(lambda p: p.requires_grad, net.encoder.parameters()), lr=encoder_lr)

    # start training
    net = net.to(device)
    for epoch in range(start_epoch, start_epoch + epoches):
        # Adjust learning rate if performance cannot improve for a long time
        if epoches_since_improvement == 20:
            break
        if epoches_since_improvement > 0 and epoches_since_improvement % 8 == 0:
            for name, optimizer in optimizers.items():
                if optimizer is not None:
                    adjust_learning_rate(optimizer, 0.8)

        train(net=net, dataloader=train_dataloader, optimizers=optimizers, device=device, epoch=epoch, print_freq=print_freq)
        ...
        if not is_best:
            epoches_since_improvement += 1
            print('\n[INFO] Epoches since last improvement: {}\n'.format(epoches_since_improvement))
        else:
            epoches_since_improvement = 0

        save_checkpoint(epoch, net=net, optimizers=optimizers, bleu4=current_bleu4, is_best=is_best, \
            epoches_since_improvement=epoches_since_improvement, out_folder='./checkpoints')
